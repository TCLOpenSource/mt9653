...
#define UNLOCK_STS_SIZE				2
#define WR_EN_CMD					0x06
#define WR_STS_CMD					0x1
#define WR_STS_CLEAN_CMD			0x0
...
{
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB0,u8Data,REG_FSP_WDB0_MASK);
}
...
static void _HAL_FSP_WriteBuf1(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	...
	Data = u8Data;
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB1,(Data<<8),REG_FSP_WDB1_MASK);
}

static void _HAL_FSP_WriteBuf2(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB2,u8Data,REG_FSP_WDB2_MASK);
}
...
static void _HAL_FSP_WriteBuf3(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	...
	Data = u8Data;
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB3,(Data<<8),REG_FSP_WDB3_MASK);
}

static void _HAL_FSP_WriteBuf4(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB4,u8Data,REG_FSP_WDB4_MASK);
}
...
static void _HAL_FSP_WriteBuf5(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	...
	Data = u8Data;
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB5,(Data<<8),REG_FSP_WDB5_MASK);
}

static void _HAL_FSP_WriteBuf6(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB6,u8Data,REG_FSP_WDB6_MASK);
}
...
static void _HAL_FSP_WriteBuf7(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	...
	Data = u8Data;
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB7,(Data<<8),REG_FSP_WDB7_MASK);
}

static void _HAL_FSP_WriteBuf8(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB8,u8Data,REG_FSP_WDB8_MASK);
}
...
static void _HAL_FSP_WriteBuf9(struct mtk_nor *mtk_nor,MS_U8 u8Data)
{
	...
	Data = u8Data;
	FSP_WRITE_MASK(mtk_nor,REG_FSP_WDB9,(Data<<8),REG_FSP_WDB9_MASK);
}
...
static int mtk_lock(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	return 0;
}

static int mtk_unlock(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	struct mtk_nor *mtk_nor = nor->priv;
	int status;
	...
	char sts_buf[UNLOCK_STS_SIZE] = {WR_STS_CMD, WR_STS_CLEAN_CMD};
	char wren_buf[1] = {WR_EN_CMD};
	...
	if (mtk_nor->type) {
		...
		if (status) {
			dev_err(mtk_nor->dev, "%s unlock wr_en status:%d\n", __func__, status);
			return -1;
		}
		spi_message_init(&message);
		memset(x, 0, sizeof(x));
		...
		x[0].speed_hz    = mtk_nor->clk;
		x[0].tx_buf = sts_buf;
		x[0].len = UNLOCK_STS_SIZE;
		spi_node = km_spidev_node_get(mtk_nor->channel);
		spi_message_add_tail(&x[0], &message);
		status = spi_sync(spi_node, &message);
		if (status) {
			dev_err(mtk_nor->dev, "%s unlock wr_sts status:%d\n", __func__, status);
			return -1;
		}
	}
	return 0;
...
static int mtk_is_locked(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	return 0;
}

static const struct spi_nor_locking_ops mtk_locking_ops = {
	.lock = mtk_lock,
	.unlock = mtk_unlock,
	.is_locked = mtk_is_locked,
};
...
static int mtk_nor_drv_probe(struct platform_device *pdev)
{
	...
	mtk_nor->nor.params.locking_ops = &mtk_locking_ops;
