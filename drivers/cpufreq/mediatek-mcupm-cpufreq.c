...
int mcupm_get_cpu_power(unsigned long *power, unsigned long *KHz, int cpu)
{
	struct mtk_cpu_dvfs_info *info = mtk_cpu_dvfs_info_lookup(cpu);
...
static int mtk_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	em_register_perf_domain(policy->cpus, opp, &em_cb);
	return 0;
