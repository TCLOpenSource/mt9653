...
static inline void _apupw_separte(struct seq_file *s, const char *separate)
{
	seq_puts(s, "\n");
...
static bool _apupw_valid_input(u8 param, int argc)
{
	...
	case POWER_PARAM_POWER_STRESS:
		...
		if (argc != expect)
			goto INVALID;
		break;
	case POWER_PARAM_PLAT_POWER_CTRL:
		if (argc == 0)
			goto INVALID;
		break;
	}
...
void apupw_dbg_power_info(struct work_struct *work)
{
	...
	if (n_pos < 0) {
		pr_info("%s string fail", __func__);
		return;
	}
...
static int apupw_dbg_dump_table(struct seq_file *s)
{
	...
	for (usr = MDLA; usr < APUSYS_POWER_USER_NUM; usr++) {
		...
		do {
			...
			dev_pm_opp_put(opp);
			len += snprintf((info + len), (sizeof(info) - len),
					"%3luMhz(%3lulumv) [%d/%3d]|", TOMHZ(freq), TOMV(volt),
...
static int apupw_dbg_show(struct seq_file *s, void *unused)
{
	...
	case POWER_PARAM_LOG_LEVEL:
		...
		break;
	case POWER_PARAM_PLAT_POWER_CTRL:
		seq_printf(s, "power_ctrl = %d\n", dtv_apupwr_get_pwrctrl());
		break;
	default:
		break;
	}
...
static int apupw_dbg_set_parameter(u8 param, int argc, int *args)
{
	...
	case POWER_PARAM_OPP_TABLE:
		...
		break;
	case POWER_PARAM_PLAT_POWER_CTRL:
		if (args[0] < DTV_PWR_CTRL_NONE)
			apupw_dbg.option = POWER_PARAM_PLAT_POWER_CTRL;
		if (args[0] == DTV_PWR_CTRL_CFG)
			ret = dtv_apupwr_set_pwrctrl(args[1], args[2]);
		break;
	default:
...
#define MAX_ARG 5
	...
	else if (!strcmp(token, "power_ctrl"))
		param = POWER_PARAM_PLAT_POWER_CTRL;
	else {
		ret = -EINVAL;
...
int apu_power_drv_init(void)
{
	/* creating apupw directory */
	apupw_dbg.dir = debugfs_create_dir("apupwr", NULL);
	if (IS_ERR_OR_NULL(apupw_dbg.dir)) {
		pr_err("failed to create \"apupwr\" debug dir.\n");
		goto out;
	}
	...
	if (IS_ERR_OR_NULL(apupw_dbg.file)) {
		pr_err("failed to create \"power\" debug file.\n");
		goto out;
	}

	/* symbolic link to /d/apupwr/power */
	apupw_dbg.sym_link = debugfs_create_symlink("power", NULL, "./apupwr/power");
	if (IS_ERR_OR_NULL(apupw_dbg.sym_link)) {
		pr_err("failed to create \"power\" symbolic file.\n");
		goto out;
	}
out:
	return 0;
