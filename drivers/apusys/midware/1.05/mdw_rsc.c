...
static int mdw_rsc_get_name(int type, char *name)
{
	...
	mdw_flw_debug("rsc dev name size = %lu/%s\n",
		(unsigned long)sizeof(rsc_dev_name)/sizeof(char *),
...
uint64_t mdw_rsc_get_avl_bmp(void)
{
	...
	bitmap_to_arr32((uint32_t *)local_b_p, bmp, APUSYS_DEVICE_MAX);
...
void mdw_rsc_update_avl_bmp(int type)
{
	...
	local_cb_p = (uint64_t *) &cb;
	local_db_p = (uint64_t *) &db;
...
//		rsc_mgr.dev_avl_bmp, APUSYS_DEVICE_MAX);
	...
	bitmap_to_arr32((uint32_t *)local_cb_p,
		rsc_mgr.cmd_avl_bmp, APUSYS_DEVICE_MAX);
	bitmap_to_arr32((uint32_t *)local_db_p,
		rsc_mgr.dev_avl_bmp, APUSYS_DEVICE_MAX);
...
static int mdw_rsc_add_dev(struct apusys_device *dev)
{
	...
	/* create kthd */
	d->thd = kthread_run(mdw_sched_dev_routine, d, "%s", (const char *)thd_name);
