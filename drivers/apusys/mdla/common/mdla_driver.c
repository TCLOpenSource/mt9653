	...
	void *hnd, struct apusys_device *dev)
{
	...
	case APUSYS_CMD_EXECUTE:
		cmd_hnd = hnd;
		cmd_data = (struct mdla_run_cmd_sync *)(uintptr_t)cmd_hnd->kva;
...
static int mdla_drv_register_char_dev(struct device *dev)
{
	...
	cdev_init(mdlactl_cdev, mdla_fops_get());
...
static int mdla_drv_create_device_node(struct device *dev)
{
	...
	/* 5. Set DMA mask */
	if (dma_get_mask(mdlactl_device) != DMA_BIT_MASK(32)) {
