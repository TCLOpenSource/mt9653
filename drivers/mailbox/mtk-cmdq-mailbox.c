...
static s32 cmdq_clk_enable(struct cmdq *cmdq)
{
	...
	err = clk_enable(cmdq->clock[0]);
	if (usage <= 0 || err < 0)
		cmdq_err("clock[0] ref count error after inc:%d err:%d suspend:%s",
			usage, err, cmdq->suspended ? "true" : "false");
	err = clk_enable(cmdq->clock[1]);
	if (usage <= 0 || err < 0)
		cmdq_err("clock[1] ref count error after inc:%d err:%d suspend:%s",
			usage, err, cmdq->suspended ? "true" : "false");
	else if (usage == 1) {
...
static void cmdq_clk_disable(struct cmdq *cmdq)
{
	...
	clk_disable(cmdq->clock[0]);
	clk_disable(cmdq->clock[1]);
...
static int cmdq_thread_suspend(struct cmdq *cmdq, struct cmdq_thread *thread)
{
	...
	if (readl_poll_timeout_atomic(thread->base + CMDQ_THR_CURR_STATUS,
			status, status & CMDQ_THR_STATUS_SUSPENDED, 0, HAPS_TIMEOUT)) {
...
static int cmdq_suspend(struct device *dev)
{
	...
	clk_unprepare(cmdq->clock[0]);
	clk_unprepare(cmdq->clock[1]);
	return 0;
}
...
static int cmdq_resume(struct device *dev)
{
	...
	WARN_ON(clk_prepare(cmdq->clock[0]) < 0);
	WARN_ON(clk_prepare(cmdq->clock[1]) < 0);
...
static int cmdq_probe(struct platform_device *pdev)
{
	...
	cmdq->clock[0] = devm_clk_get(dev, "clk_vdec_en_gce_f26m2gce_biu");
	if (IS_ERR(cmdq->clock[0])) {
		cmdq_err("failed to get clk_vdec_en_gce_f26m2gce_biu clk");
		cmdq->clock[0] = NULL;
	}

	cmdq->clock[1] = devm_clk_get(dev, "clk_vdec_en_smi2gce");
	if (IS_ERR(cmdq->clock[1])) {
		cmdq_err("failed to get clk_vdec_en_smi2gce clk");
		cmdq->clock[1] = NULL;
	}
	...
	clk_enable(cmdq->clock[0]);
	clk_enable(cmdq->clock[1]);
	cmdq_init(cmdq);
...
static const struct of_device_id cmdq_of_ids[] = {
	...
	{.compatible = "mediatek,mailbox-gce", .data = (void *)&gce_plat_v2},
	{}
};
