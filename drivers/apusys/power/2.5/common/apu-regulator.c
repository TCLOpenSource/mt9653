...
static void apu_mdla_restore_default_opp(struct work_struct *work)
{
	...
	if (ret) {
		argul_err(ad->dev, "[%s] apu_get_recommend_freq_volt fail\n", __func__);
		return;
	}
				  ...
				  unsigned long event, void *data)
{
	...
	pr_info("[%s] receive event:%lu VOL_CHANGE: %d\n", __func__,
			event, REGULATOR_EVENT_PRE_VOLTAGE_CHANGE);
...
static const struct apu_regulator_array apu_rgul_gps[] = {
	...
	{ .name = "mt688x_mdla0", .argul_gp = &mt688x_mdla_rgul_gp },
};
