	...
	int idx)
{
	...
	mdw_flw_debug("type(%u) ofs(%u) size(%lu/%lu) cmd size(%u)\n",
		sh->type,
		ofs, (unsigned long)sizeof(struct apu_sc_hdr_cmn),
		(unsigned long)sizeof(struct apu_append_hdr), cmd->size);
	switch (sh->type) {
	case APUSYS_DEVICE_SAMPLE:
	case APUSYS_DEVICE_VPU:
		if (ofs + sizeof(struct apu_sc_hdr_cmn) +
			sizeof(struct apu_append_hdr) > cmd->size)
			goto fail_size;
		break;
...
static inline int mdw_cmd_sc_valid(struct mdw_apu_sc *sc)
{
	...
	if (sc->hdr->mem_ctx >= MDW_CMD_SC_MAX - 1 &&
		sc->hdr->mem_ctx != VALUE_LEGACY_PACK_NONE) {
		mdw_drv_err("sc(0x%llx-#%d) invalid ctx(%u)\n",
			c->kid, sc->idx, sc->hdr->mem_ctx);
		return -EINVAL;
	}
	/* check pack id */
	if (sc->pack_id >= MDW_CMD_SC_MAX) {
		mdw_drv_err("sc(0x%llx-#%d) invalid pack(%u)\n",
			c->kid, sc->idx, sc->pack_id);
		return -EINVAL;
	}
...
static inline uint8_t mdw_cmd_sc_get_pack(struct apu_sc_hdr_cmn *sh)
{
	struct apu_append_hdr *ah = NULL;
	uint8_t pack_id = 0;

	switch (sh->type) {
	case APUSYS_DEVICE_SAMPLE:
	case APUSYS_DEVICE_VPU:
		ah = (struct apu_append_hdr *)((uint64_t)sh +
			sizeof(struct apu_sc_hdr_cmn));
		pack_id = (uint8_t)ah->pack_id;
		break;
	default:
		break;
	}
...
static int mdw_cmd_get_pack_ctx(struct mdw_apu_cmd *c)
{
	...
	uint8_t pack_list[MDW_CMD_SC_MAX];
	uint8_t p_id = 1, tmp_start = 0, tmp = 0;

	memset(c->ctx_repo, MDW_CMD_EMPTY_NUM, sizeof(c->ctx_repo));
	memset(pack_list, 0, sizeof(pack_list));

	/* get origin pack id from hdr */
	for (i = 0; i < c->hdr->num_sc; i++) {
		h = mdw_cmd_get_sc_hdr(c, i);
		if (!h)
			return -EINVAL;

		pack_list[i] = mdw_cmd_sc_get_pack(h);
		if (pack_list[i] >= MDW_CMD_SC_MAX && pack_list[i] !=
			0xff)
			return -EINVAL;
	}
	...
	for (i = 0; i < c->hdr->num_sc; i++) {
		...
		if (!h)
			return -EINVAL;
		/* ctx */
		if (h->mem_ctx <= MDW_CMD_SC_MAX - 1)
			c->mod_ctx[i] = h->mem_ctx + 1;
		else if (h->mem_ctx == VALUE_LEGACY_CTX_NONE)
			c->mod_ctx[i] = VALUE_SUBGRAPH_CTX_ID_NONE;
		else
			return -EINVAL;
		...
		tmp_start = pack_list[i];
		tmp = tmp_start;
		do {
			if (tmp >= MDW_CMD_SC_MAX ||
				tmp > c->hdr->num_sc)
				return -EINVAL;
			barrier();
			c->mod_pack[tmp] = p_id;
			mdw_cmd_debug("idx(%u) set mod (%u)pack(%u->%u)\n",
				i, tmp, pack_list[tmp], p_id);
			tmp = pack_list[tmp];
		} while (tmp != tmp_start);
		p_id++;

		barrier();
		mdw_cmd_debug("idx(%u) ctx(%u->%u) mod pack(%u->%u)\n",
			i, h->mem_ctx, c->mod_ctx[i],
			pack_list[i], c->mod_pack[i]);
	}

	for (i = 0; i < c->hdr->num_sc; i++) {
		c->ctx_cnt[c->mod_ctx[i]]++;
		c->pack_cnt[c->mod_pack[i]]++;
	}
	...
	uint32_t size, uint32_t ofs, struct mdw_usr *u)
{
	...
	if (ofs + sizeof(struct apu_cmd_hdr) > size) {
		mdw_drv_err("hdr ofs overflow(%u/%lu/%u)\n",
			ofs, (unsigned long)sizeof(struct apu_cmd_hdr), size);
		goto out;
	}
...
static struct mdw_apu_sc *mdw_cmd_create_sc(struct mdw_apu_cmd *c)
{
	...
	mdw_cmd_debug("sc(0x%llx-#%d) ctx(%d/%u) pack(%u)\n",
		c->kid, sc->idx, sc->hdr->mem_ctx,
		c->mod_ctx[sc->idx], sc->pack_id);
...
int mdw_cmd_get_ctx(struct mdw_apu_sc *sc)
{
	...
	if (c->mod_ctx[sc->idx] == VALUE_SUBGRAPH_CTX_ID_NONE &&
		sc->multi_total <= 1) {
#ifndef AI_SIM
		ret = reviser_get_vlm(tcm_usage, sc->hdr->tcm_force,
			&sc->ctx, &sc->real_tcm_usage);
#endif
		mdw_flw_debug("sc(0x%llx-#%d) get ctx(%lu/%u/%u/%u)\n",
			c->kid, sc->idx, sc->ctx, sc->hdr->mem_ctx,
			c->mod_ctx[sc->idx], tcm_usage);
		goto out;
	}

	if (c->ctx_repo[c->mod_ctx[sc->idx]] != MDW_CMD_EMPTY_NUM) {
		sc->ctx = (uint32_t)c->ctx_repo[c->mod_ctx[sc->idx]];
		goto out;
	}
	...
	ret = reviser_get_vlm(tcm_usage, sc->hdr->tcm_force,
		&sc->ctx, &sc->real_tcm_usage);
#endif
	c->ctx_repo[c->mod_ctx[sc->idx]] = sc->ctx;
	mdw_flw_debug("sc(0x%llx-#%d) get ctx(%lu/%u/%u/%u)\n",
		c->kid, sc->idx, sc->ctx, sc->hdr->mem_ctx,
		c->mod_ctx[sc->idx], tcm_usage);
...
#endif
		mdw_flw_debug("sc(0x%llx-#%d) put ctx(%lu/%u/%u)\n",
			c->kid, sc->idx, sc->ctx,
			sc->hdr->mem_ctx, c->mod_ctx[sc->idx]);
		goto out;
	}

	c->ctx_cnt[c->mod_ctx[sc->idx]]--;
	if (!c->ctx_cnt[c->mod_ctx[sc->idx]]) {
#ifndef AI_SIM
		reviser_free_vlm(sc->ctx);
#endif
		c->ctx_repo[c->mod_ctx[sc->idx]] = MDW_CMD_EMPTY_NUM;
		mdw_flw_debug("sc(0x%llx-#%d) put ctx(%lu/%u/%u)\n",
			c->kid, sc->idx, sc->ctx,
			sc->hdr->mem_ctx, c->mod_ctx[sc->idx]);
	}
