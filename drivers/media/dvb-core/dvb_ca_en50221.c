				      ...
				      unsigned int cmd, void *parg)
{
	...
	case CA_GET_SLOT_INFO: {
		...
		struct dvb_ca_slot *sl;
#ifdef CONFIG_DVB_CA_EN50221_SLOT_INFO_CHECK_STATUS
		int status;
#endif
		...
		if ((sl->slot_state != DVB_CA_SLOTSTATE_NONE) &&
			...
			/* If the slot is not in the normal state, it is
			 *necessary to check the slot status from driver.
			 */
			if (ca->pub->poll_slot_status) {
				status = ca->pub->poll_slot_status(ca->pub, slot, 0);
