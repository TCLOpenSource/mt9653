	...
	*((struct atsc3t_plp_bitmap *)&tvp->u.buffer.data) = valid_bitmap;
	...
	case DTV_ATSC3T_PLP_STATUS:{
		...
		for (i = 0; i < DVB_AVAILABLE_PLD_ID_NUM; i++) {
			llsFlag =
			((__u64) 1 << dvb_plp_lock_target_global[i]) & (c->llsFlag_bitmap.bitmap);
			if ((dvb_plp_lock_target_global[i] != 0xFF) && (llsFlag != 0)) {
				if (dvb_plp_lock_status_global[i] == 0) {
					dvb_plp_lock_status_global[i] = FE_HAS_LOCK;
					break;
				}
			}
		}
		tvp->u.buffer.len = DVB_AVAILABLE_PLD_ID_NUM;
		memcpy(tvp->u.buffer.data, dvb_plp_lock_status_global,
			sizeof(dvb_plp_lock_status_global));
		break;
	}
