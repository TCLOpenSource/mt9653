...
TRACE_EVENT(apupwr_dvfs,
	...
	TP_fast_assign(
		strncpy(__get_str(gov_name), gov_name, strlen(gov_name));
		strncpy(__get_str(p_name), p_name, strlen(p_name));
		strncpy(__get_str(c_name), c_name, strlen(c_name));
		__entry->opp = opp;
		__entry->freq = freq;
	),
	TP_printk("[%s] %s->%s[%u/%lu]",
