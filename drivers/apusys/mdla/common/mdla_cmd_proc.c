...
static int mdla_cmd_dummy_run(struct mdla_run_cmd_sync *run_cmd,
			...
			bool secure, bool enable_preempt)
{
	mdla_cmd_debug("%s() !!!\n", __func__);
	return 0;
}

static int mdla_cmd_dummy_ut_run_sync(void *cmd_data, struct mdla_dev *mdla_info)
{
	mdla_cmd_debug("%s() !!!\n", __func__);
	return 0;
}
...
static int mdla_cmd_dummy_ce_ops_str(u32 a0,
	struct command_entry *a1, const char *str)
{
	return 0;
}
...
static struct mdla_cmd_cb_func mdla_command_callback = {
	...
	.add_time_stamp		= mdla_cmd_dummy_ce_ops_str,
	.print_time_stamp	= mdla_cmd_dummy_ce_ops,
	.suspend_cb          = mdla_cmd_dummy_ops,
	.resume_cb          = mdla_cmd_dummy_ops,
	.check_resource          = mdla_cmd_dummy_ops,
};
