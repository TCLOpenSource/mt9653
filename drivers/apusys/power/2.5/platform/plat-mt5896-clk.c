...
static void __iomem *reg_0x102f;
...
static int ioremap_init(void)
{
	...
	reg_0x102f = ioremap_nocache(RIU2IOMAP(0x102f, 0), PAGE_SIZE);
	if (!reg_ckgen00
		|| !reg_0x100c
		|| !reg_0x102a
		|| !reg_0x102f)
...
static void mt5896_clk_disable(struct apu_clk_gp *aclk)
{
	//cg/sw enable
}
...
static unsigned long mt5896_clk_get_rate(struct apu_clk_gp *aclk)
{
	return curr_rate;
}
...
static void buspll_set_rate(unsigned long rate)
{
	...
	riu_write16(0x102f, 0x54, 0x0000);
	riu_write16(0x102f, 0x56, 0x0400);
	riu_write16(0x102f, 0x57, 0x0000);
	riu_write16(0x102f, 0x58, 0x0034);
	riu_write16(0x102f, 0x52, 0x0000);
	riu_write16(0x102f, 0x51, 0x0001);
	riu_write16(0x102f, 0x50, 0x1110);
	udelay(50);
	riu_write16(0x102f, 0x51, 0x0000);
	riu_write16(0x102f, 0x50, 0x0000);
}

static int mt5896_clk_set_rate(struct apu_clk_gp *aclk, unsigned long rate)
{
	struct apu_clk *dst;
...
{
	riu_write16(0x102f, 0x58, 0x003c);
	riu_write16(ckgen00, 0x3c, 0x001c);
	riu_write16(ckgen00, 0x40, 0x001c);
	riu_write16(ckgen00, 0x44, 0x0000);
	riu_write16(0x102a, 0x2b, 0x0003);
}

static int mt5896_clk_enable(struct apu_clk_gp *aclk)
{
	unsigned long rate;
...
static struct apu_clk mt5896_mdla0_topmux = {
	.always_on = 1,
	.fix_rate = 1,
};

static struct apu_cg mt5896_mdla0_cg[] = {
	{
		.phyaddr = 0x19038000,
		.cg_ctl = {0, 4, 8},
	},
...
static struct apu_cgs mt5896_mdla0_cgs = {
	.cgs = &mt5896_mdla0_cg[0],
	.clk_num = ARRAY_SIZE(mt5896_mdla0_cg),
};

static struct apu_clk_ops mt5896_mdla0_clk_ops = {
	.prepare	= dtv_apu_dummy_clk_prepare,
	.unprepare	= dtv_apu_dummy_clk_unprepare,
	.enable		= mt5896_clk_enable,
	.disable	= mt5896_clk_disable,
	.cg_enable	= dtv_apu_dummy_cg_enable,
	.cg_status	= dtv_apu_dummy_cg_status,
	.set_rate	= mt5896_clk_set_rate,
	.get_rate	= mt5896_clk_get_rate,
};

static struct apu_clk_ops mt5896_mdla_clk_ops = {
	.prepare	= dtv_apu_dummy_clk_prepare,
	.unprepare	= dtv_apu_dummy_clk_unprepare,
	.enable		= mt5896_clk_enable,
	.disable	= mt5896_clk_disable,
...
static struct apu_clk_gp mt5896_mdla0_clk_gp = {
	.ops = &mt5896_mdla0_clk_ops,
};

static struct apu_clk_gp mt5896_mdla_clk_gp = {
	.top_mux = &mt5896_mdla0_topmux,
	.cg = &mt5896_mdla0_cgs,
	.ops = &mt5896_mdla_clk_ops,
};

static const struct apu_clk_array apu_clk_gps[] = {
	[MDLA] = { .name = "mt589x_mdla", .aclk_gp = &mt5896_mdla_clk_gp },
	[MDLA0] = { .name = "mt589x_mdla0", .aclk_gp = &mt5896_mdla0_clk_gp },
};
