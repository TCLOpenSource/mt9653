...
static int update_parent(struct apu_gov_data *gov_data)
{
	...
	apu_flw_debug("[%s] dev: %s\n", __func__, apu_dev_name(gov_data->parent->dev.parent));
...
static int agov_get_target_freq(struct devfreq *df, unsigned long *freq)
{
	...
	if (ad->plat_ops->get_target_freq)
		*freq = ad->plat_ops->get_target_freq(ad);
	else
		*freq = apu_opp2freq(ad, req->value);

	apu_flw_debug("[%s] dev: %s, target: %luMHz\n", __func__,
		apu_dev_name(ad->dev), TOMHZ(*freq));
