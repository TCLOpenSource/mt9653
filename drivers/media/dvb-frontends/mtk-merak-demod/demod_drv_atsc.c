...
ssize_t atsc_get_information(char *buf)
{
	...
	if (data1 == 0)
		interleaving_mode = INTERLEAVING_128_1_0;
	else if (data1 == 1)
		interleaving_mode = INTERLEAVING_128_1_1;
	else if (data1 == 3)
		interleaving_mode = INTERLEAVING_64_2;
	else if (data1 == 5)
		interleaving_mode = INTERLEAVING_32_4;
	else if (data1 == 7)
		interleaving_mode = INTERLEAVING_16_8;
	else if (data1 == 9)
		interleaving_mode = INTERLEAVING_8_16;
	else if (data1 == 2)
		interleaving_mode = INTERLEAVING_128_2;
	else if (data1 == 4)
		interleaving_mode = INTERLEAVING_128_3;
	else if (data1 == 6)
		interleaving_mode = INTERLEAVING_128_4;
	else
		interleaving_mode = INTERLEAVING_NONE;
	...
	if (data1 == 0x02)
		etype = DMD_ATSC_DEMOD_ATSC_256QAM;
	else if (data1 == 0x04)
		etype = DMD_ATSC_DEMOD_ATSC_VSB;
	else
		etype = DMD_ATSC_DEMOD_ATSC_64QAM;

	return scnprintf(buf, PAGE_SIZE,
	"%s%s%x%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%sd %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s",
	SM_CHIP_REV, SM_COMMA, dmd_rev, SM_END,
	...
	SM_INTERLEAVING, SM_COMMA, interleaving_mode, SM_END,
	SM_MODULATION, SM_COMMA, etype, SM_END,
	...
	SD_STATUS, SM_COMMA, fgdemodStatus, SM_END);
}
#ifdef UTPA2
