...
#ifdef CMDQ_SECURE_TEST_SUPPORT
	if (sec)
	...
	struct cmdq_client *clt, const char *tag)
{
	...
	pkt = cmdq_pkt_create(test->clt);
	cmdq_pkt_write_value_addr(pkt, pa, pat_src, ~0);
	cmdq_pkt_flush(pkt);

	if (va[0] != pat_src) {
		cmdq_msg(
			"%s access dram fail pa:%pa val:%#x should be:%#x ",
			tag, &pa, va[0], pat_src);
	}
	else {
		cmdq_msg(
			"%s access dram pass", tag, &pa);
	}
...
#define IO_WRITE32(base, offset, value) do { HAL_WRITE32((base) + (offset), (value)); _dsb(); } while(0)
...
static int cmdq_test_probe(struct platform_device *pdev)
{
	...
	sleep_protect_va = of_iomap(np_pdev->dev.of_node, 1);
	IO_WRITE32(sleep_protect_va, 0x20, 0x84);
	IO_WRITE32(sleep_protect_va, 0x0, 0x220007);
