...
static int _enhance_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_DOLBY_BIN_BUFFER: //9964088
		ret = mtk_hdr_SetDVBinDone(hdr_info, ctrl->p_new.p,
	...
	case V4L2_CID_PQMAP_INFO:
		ret = mtk_pqd_SetPQMapInfo(pqd_info, ctrl->p_new.p,
			pqdev->DDBuf[MMAP_PQMAP_INDEX].va, pqdev->DDBuf[MMAP_PQMAP_INDEX].size);
		break;
	case V4L2_CID_PQ_METADATA:
		ret = mtk_pqd_SetPQMetaData(pqd_info, ctrl->p_new.p);
		break;
	case V4L2_CID_PQPARAM:
		ret = mtk_pqd_SetPQParam(pqd_info, ctrl->p_new.p,
			pqdev->DDBuf[MMAP_PQPARAM_INDEX].va, pqdev->DDBuf[MMAP_PQPARAM_INDEX].pa,
