...
static int mdla_pwr_dummy_set_pll(u32 core_id, u64 state)
{
	return 0;
}

static int mdla_pwr_dummy_get_pll(u32 core_id, u64 *max,
					u64 *min, u64 *curr)
{
	return 0;
}
...
static struct mdla_pwr_ops mdla_power = {
	...
	.set_pll            = mdla_pwr_dummy_set_pll,
	.get_pll            = mdla_pwr_dummy_get_pll,
};
...
struct mdla_pwr_ops *mdla_pwr_ops_get(void)
{
	return &mdla_power;
}
			...
			int (*off)(u32 core_id, int suspend, bool force))
{
	...
	for_each_mdla_core(i) {
		...
		pwr_ctrl = kmalloc(sizeof(struct mdla_pwr_ctrl),
					GFP_KERNEL);
		if (!pwr_ctrl)
			goto out;

		memset(pwr_ctrl, 0, sizeof(*pwr_ctrl));
		pwr_ctrl->mdla_id = i;
