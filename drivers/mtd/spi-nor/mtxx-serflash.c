...
static void HAL_FSP_WriteBufs(struct mtk_nor *mtk_nor,MS_U8 u8Idx, MS_U8 u8Data)
{
	...
	case 9:
		...
		break;
	default:
		dev_err(mtk_nor->dev,"HAL_FSP_WriteBufs fails\n");
		break;
	}
}
...
static MS_BOOL HAL_FSP_Write(struct mtk_nor *mtk_nor,MS_U32 u32Addr, MS_U32 u32Size, MS_U8 *pu8Data)
{
	...
	for(u32J = 0; u32J <= u32quotient; u32J++) {
		...
		if(u8Status)
			dev_err(mtk_nor->dev, "HAL_FSP_Write HAL_FSP_CheckFlashBusy fail\n");
			    ...
			    u_char *buffer)
{
	...
	if (mtk_nor->type) {
		...
		if (status) {
			dev_err(mtk_nor->dev, "mtk_nor_read fail status:%d\n",status);
			return -1;
		}
	} else {
		HAL_FSP_Entry(mtk_nor);
...
static int mtk_nor_drv_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node,
				"nor-clk", &mtk_nor->clk);
	if (ret) {
		dev_err(&pdev->dev, "could not get clk_div\n");
		mtk_nor->clk = NOR_CLK;
	}
	ret = of_property_read_u32(pdev->dev.of_node,
				"mspi-channel", &mtk_nor->clk);
	if (ret) {
		dev_err(&pdev->dev, "could not get mspi channel\n");
		mtk_nor->channel = MSPI_CH;
	}
	ret = of_property_read_u32(pdev->dev.of_node,
				"nor-type", &mtk_nor->type);
	if (ret) {
		dev_err(&pdev->dev, "could not get mspi nor-type\n");
		mtk_nor->type = 0;
	} else {
